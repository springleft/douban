<template>
  <div class="talion-view">
    <div class="header-bar">
      <span class="close-talion" @click="closeTalion">关闭</span>
      <form class="search" onsubmit="return false">
        <input
          type="search"
          name="query"
          v-model.trim.lazy="queryStr"
          @keyup.enter="goSearch()">
      </form>
    </div>
    <ul class="has-header">
      <li>
        <div>
          <a href="http://127.0.0.1:1019/#/movie" target="_blank">
            <strong style="color: rgb(35, 132, 232);">电影</strong>
            <span>影院热映</span>
          </a>
          <a href="https://beijing.douban.com/" target="_self">
            <strong style="color: rgb(230, 70, 126);">同城</strong>
            <span>周末活动</span>
          </a>
          <a href="https://read.douban.com/" target="_self">
            <strong style="color: rgb(159, 120, 96);">阅读</strong>
            <span>电子书</span>
          </a>
          <a href="https://dongxi.douban.com/" target="_self">
            <strong style="color: rgb(225, 100, 77);">东西</strong>
            <span>心爱之物</span>
          </a>
        </div>
      </li>
      <li>
        <div>
          <a href="https://m.douban.com/tv" target="_blank">
            <strong style="color: rgb(122, 106, 219);">电视</strong>
            <span>正在热播</span>
          </a>
          <a href="https://www.douban.com/group/explore" target="_blank">
            <strong style="color: rgb(42, 184, 204);">小组</strong>
            <span>志趣相投</span>
          </a>
          <a href="https://m.douban.com/game/" target="_blank">
            <strong style="color: rgb(87, 116, 197);">游戏</strong>
            <span>虚拟世界</span>
          </a>
          <a href="https://douban.fm/user-guide" target="_self">
            <strong style="color: rgb(64, 207, 169);">FM</strong>
            <span>红心歌单</span>
          </a>
        </div>
      </li>
      <li>
        <div>
          <a href="http://127.0.0.1:1019/#/book" target="_blank">
            <strong style="color: rgb(159, 120, 96);">图书</strong>
            <span>畅销排行</span>
          </a>
          <a href="https://m.douban.com/music/" target="_blank">
            <strong style="color: rgb(244, 143, 46);">音乐</strong>
            <span>新碟榜</span>
          </a>
          <a href="https://m.douban.com/mobileapp/" target="_blank">
            <strong style="color: rgb(89, 108, 221);">应用</strong>
            <span>玩手机</span>
          </a>
          <a href="https://market.douban.com" target="_self">
            <strong style="color: rgb(66, 189, 86);">市集</strong>
            <span>购买原创</span>
          </a>
        </div>
      </li>
    </ul>

    <div class="nav-bottom">
          <div>
            <router-link to="/register" >注册账号</router-link>
            <router-link to="/login">登录豆瓣</router-link>
          </div>
         <div>
          <a href="https://movie.douban.com/">使用桌面版</a>
          <a href="#">使用豆瓣APP</a>
        </div>
    </div>
  </div>
</template>

<script>

export default {
  name: 'talion-view',
  data () {
    return {
      queryStr: ''
    }
  },
  methods: {
    closeTalion: function () {
      
      $('.talion-view').hide();
    },
    goSearch: function () {
      $('.talion-view').hide();
      this.$router.push({
        name: 'Search',
        params: {
          q: this.queryStr
        }
      })
    }
  }
}
</script>

<style  scoped>
.talion-view{
    display: none;
    height: 100%;
    position: absolute;
    bottom: 0;
    -webkit-transform: translateZ(0);
    transform: translateZ(0);
    position: fixed;
    left: 0;
    right: 0;
    top: 0;
    z-index: 9999;
    background: #fff;

}
.close-talion {
      color: #42bd56;
    font-size: 1.6rem;
}
.search {
    flex: 1;
    width: 100%;
    padding-left: 1rem;
}
.header-bar{
    position: fixed;
    left: 0;
    right: 0;
    top: 0;
    z-index: 998;
    display: -ms-flexbox;
    display: flex;
    -ms-flex-align: center;
    align-items: center;
    height: 4.8rem;
    padding: 0 1.8rem;
    background: #fff;
    border-bottom: 0.1rem solid #f3f3f3;
}
input[type=search] {
  box-sizing: border-box;
  border: 0;
  background: url(data:image/png;base64,) no-repeat 50% #f3f3f3;
  background-size: 1.73rem;
  border-radius: 0.3rem;
  width: 100%;
  height: 3.2rem;
  font-size: 1.4rem;
}

input[type=search]:focus {
  background: #f3f3f3;
}
.has-header {
  display: flex;
  justify-content: space-around;
  padding-top: 2.5rem;
  margin-top:4.8rem; 
}

 .has-header>li {
    width: 33.33%;
  }

 .has-header div {
    position: relative;
    float: left;
    left: 50%;
    top: 0;
  }

 .has-header a {
    position: relative;
    left: -50%;
    top: 0;
    display: block;
    margin-bottom: 2.5rem;
    text-align: center;
  }

 .has-header strong {
    display: block;
    font-weight: 400;
    font-size: 2.4rem;
    line-height: 2.8rem;
  }

  .has-header span {
    display: block;
    color: #999;
    height: 1.7rem;
    font-size: 1.2rem;
    letter-spacing: 0.1rem;
  }
.nav-bottom{
    width: 100%;
    border-bottom: 0.1rem solid #f3f3f3;
}
.nav-bottom>div{
    border-top: 1px solid #f3f3f3;
    padding: 13px 0;
    text-align: center;
    display: flex;
}
.nav-bottom a{
    flex: 1;
    text-align: center;
    text-decoration: none;
    font-size: 15px;
    color: #42bd56;
    box-sizing: border-box;
    border-right: 1px solid #e5e5e5;
}
</style>
